...
if __name__ == '__main__':
    ...
    # Gragh = networks.build_dynamic_network_from_txt(dataset_file_name, train_start_timestamp, train_end_timestamp)
    ...
    print(neighbors_of_chosen_node)
    ...
    for node in Graph_new.nodes:
        if (node not in neighbors_of_chosen_node) and (node != chosen_node):
            ...
            # evolute
            Graph_new = networks.add_edges_to_graph_from_list(Graph_new, predict_list)
            ...
            Graph_new = networks.remove_edges_from_graph_from_list(Graph_new, predict_list)
